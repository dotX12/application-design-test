...
func New(config ...Config) fiber.Handler {
	...
	return func(c *fiber.Ctx) error {
		...
		case zerolog.DebugLevel:
			logger.Debug().Ctx(c.UserContext()).Msg(message)
		case zerolog.InfoLevel:
			logger.Info().Ctx(c.UserContext()).Msg(message)
		case zerolog.WarnLevel:
			logger.Warn().Ctx(c.UserContext()).Msg(message)
		case zerolog.ErrorLevel:
			logger.Error().Ctx(c.UserContext()).Msg(message)
		case zerolog.FatalLevel:
			logger.Fatal().Ctx(c.UserContext()).Msg(message)
		case zerolog.PanicLevel:
			logger.Panic().Ctx(c.UserContext()).Msg(message)
		case zerolog.TraceLevel:
			logger.Trace().Ctx(c.UserContext()).Msg(message)
		default:
			logger.Info().Ctx(c.UserContext()).Msg(message)
		}
		return nil
